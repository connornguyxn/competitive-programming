...
int main() {
    ...
    ////////////////////
    unordered_map<str, int> ans{{"012345678", 0}};
    deque<pos> q{{"012345678", 0, 0}};
    ...
    while (q.size()) {
        ...
        Rep(d, 4) {
            ...
            swap(nxt.s[nxt.x * 3 + nxt.y], nxt.s[cur.x * 3 + cur.y]);
            if (ans.count(nxt.s)) continue;
            ans[nxt.s] = ans[cur.s] + 1;
            q.push_front(nxt);
        }
    }
